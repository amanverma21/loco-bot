...
setInterval(() => {
  http.get(`http://$https://darjicd-trivia-plus--1.glitch.me/`);
}, 280000);
...
bot.login("NTYxMDc2NTg1MzE5NDk3NzI4.D39WbA.1vatvhVuLSCEeIso8SWffZTVLEE",);
...
/** @type {!Array} */
var bbchannels = ["560779699828293635","557439829218951168"];
/** @type {!Array} */
...
/** @type {!Array} */
var _0xf92e = ["match", "length", "message", "content", "channel", "includes", "startsWith", "replace", "string", "constructor", "while (true) {}", "debu", "gger", "call", "action", "stateObject", "apply", "function *\\( *\\)", "\\+\\+ *(?:_0x(?:[a-f0-9]){4,6}|(?:\\b|\\d)[a-z0-9]{1,4}(?:\\b|\\d))", "init", "test", "chain", "input", "channels", "get", "send", "TRIVIA HUNGAMA", "Search for HQ in progress.", "TRIVIA HUNGAMA || SHIVAM ", "https://cdn.discordapp.com/attachments/551029935595061249/551030127106981908/pHAMA2K9.jpg", 
"__**Answer 2**__", "__**Answer 3**__", "LOCO ANSWER", "__**Answer 1**__", "Search for Loco in progress.", "fetchMessages", "user", "then", "first", "edit", "log", "Time is up!", "lastMessage", "shift", "push", "0x0", "0x1", "0x2", "i", "0x3", "0x5", "0x4", "0x6", "0", "0xa", "0xb", "0xc", "0xd", "0xe", "0xf", "0x9", "0x8", "0x7", "0x10", "0x11", "0x12", "0x16", "0x17", "0x15", "id", "0x14", "0x13", "0x19", "0x18", "0x1a", "0x1c", "0x1b", "0x1d", "0x1e", "0x1f", 
...
bot.on("message", async message =>{
	...
	if (cmd ===`${prefix}end` || cmd ===`${prefix}end`) {
                 ...
                 message.delete(0)
		 message.channel.send("```Loco Has Now ended```").then(function (message) {
              message.react("??")
            });
		}
         
        if (cmd ===`${prefix}lo` || cmd ===`${prefix}lo`) {
    
                message.delete(0)}
        
        if(cmd === `${prefix}san` || cmd ===`${prefix}san`){
        ...
        if (cmd ===`${prefix}bb` || cmd ===`${prefix}bb`) {
    
                message.delete(0)}

        if (cmd ===`${prefix}hq` || cmd ===`${prefix}hq`) {
    ...
    m.edit(`Pong! Latency is ${m.createdTimestamp - message.createdTimestamp}ms. API Latency is ${Math.round(client.ping)}ms`);
        ...
        if (cmd ===`${prefix}start` || cmd ===`${prefix}start`) {
                 message.delete(0)
		 message.channel.send("```Loco is Starting Now - headcount```").then(function (message) {
              message.react("??")
            });
               }
